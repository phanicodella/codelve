...
namespace codelve {
    namespace core {
        ...
        ContextManager::ContextManager(std::shared_ptr<utils::Config> config)
            ...
            LOG_INFO("ContextManager: Initialized with max context size: " +
                std::to_string(maxContextSize_));
        }

        ContextManager::~ContextManager() {
            LOG_INFO("ContextManager: Destroyed");
        }
        ...
        bool ContextManager::initialize(const scanner::IndexedCode& indexedCode) {
            try {
                ...
                LOG_INFO("ContextManager: Initialized with " +
                ...
                LOG_ERROR("ContextManager: Failed to initialize with indexed code: " +
                    std::string(e.what()));
                return false;
            }
        }
        ...
        std::string ContextManager::buildContext(const std::string& query) {
            ...
            if (result.length() > maxContextSize_ * 4) {  // Approximate character to token ratio
                ...
                LOG_WARNING("ContextManager: Context truncated to fit token limit");
            }

            LOG_INFO("ContextManager: Built context with " + std::to_string(result.length()) + " characters");
        ...
        void ContextManager::clearHistory() {
            history_.clear();
            LOG_INFO("ContextManager: Conversation history cleared");
        }
