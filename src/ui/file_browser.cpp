...
namespace codelve {
    namespace ui {
        ...
        FileBrowser::FileBrowser(HWND parentWindow, std::shared_ptr<utils::Config> config)
            ...
            utils::Logger::staticLog(utils::LogLevel::INFO, "FileBrowser: Creating");
        }

        FileBrowser::~FileBrowser() {
            utils::Logger::staticLog(utils::LogLevel::INFO, "FileBrowser: Destroying");
        ...
        bool FileBrowser::initialize() {
            ...
            if (!RegisterClassEx(&wcex)) {
                utils::Logger::staticLog(utils::LogLevel::ERROR, "FileBrowser: Failed to register browser class");
                return false;
            }
            ...
            if (!browserContainer_) {
                utils::Logger::staticLog(utils::LogLevel::ERROR, "FileBrowser: Failed to create browser container");
                return false;
            }
            ...
            utils::Logger::staticLog(utils::LogLevel::INFO, "FileBrowser: Initialized");
            return true;
        }
        ...
        bool FileBrowser::setRootDirectory(const std::string& rootPath) {
            ...
            if (!fs::exists(rootPath) || !fs::is_directory(rootPath)) {
                utils::Logger::staticLog(utils::LogLevel::ERROR,
        ...
        std::string FileBrowser::getItemPath(HTREEITEM item) const {
            ...
            TV_ITEM tvi = { 0 };  // Initialize the struct
            tvi.mask = TVIF_PARAM;
            tvi.hItem = item;
        ...
        void FileBrowser::handleItemSelect(HTREEITEM item) {
            ...
            if (!path.empty() && fs::is_directory(path)) {
                ...
                TVITEM tvi = { 0 };  // Initialize the struct
