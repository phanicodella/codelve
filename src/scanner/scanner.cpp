...
namespace codelve {
    namespace scanner {
        ...
        Scanner::Scanner(std::shared_ptr<utils::Config> config)
            ...
            logger.log(utils::LogLevel::INFO, "Scanner: Initialized with " +
        ...
        Scanner::~Scanner() {
            utils::Logger& logger = utils::Logger::getInstance();
            logger.log(utils::LogLevel::INFO, "Scanner: Destroyed");
        }
        ...
        IndexedCode Scanner::scanDirectory(const std::string& directoryPath) {
            ...
            if (!fs::exists(directoryPath) || !fs::is_directory(directoryPath)) {
                utils::Logger& logger = utils::Logger::getInstance();
                logger.log(utils::LogLevel::ERROR, "Scanner: Invalid directory path: " + directoryPath);
                return indexedCode;
            }

            utils::Logger& logger = utils::Logger::getInstance();
            logger.log(utils::LogLevel::INFO, "Scanner: Starting scan of directory: " + directoryPath);
            ...
            try {
                ...
                for (const auto& entry : fs::recursive_directory_iterator(directoryPath)) {
                    ...
                    if (entry.is_directory() &&
                        ...
                        logger.log(utils::LogLevel::DEBUG, "Scanner: Skipping excluded directory: " + currentPath.string());
                        continue;
                    }
                }
                ...
                for (const auto& entry : fs::recursive_directory_iterator(directoryPath)) {
                    ...
                    if (fileSize > maxFileSize_) {
                        logger.log(utils::LogLevel::DEBUG, "Scanner: Skipping large file: " + entry.path().string());
                        continue;
                    }
                    ...
                    if (!file) {
                        logger.log(utils::LogLevel::WARNING, "Scanner: Failed to open file: " + entry.path().string());
                        continue;
                    }
                    ...
                    if (lineCount > maxLineCount_) {
                        logger.log(utils::LogLevel::DEBUG,
                    ...
                    if (processedFiles >= maxFileCount_) {
                        logger.log(utils::LogLevel::WARNING, "Scanner: Reached maximum file count limit");
                        break;
                    }
                }
                ...
                logger.log(utils::LogLevel::INFO, "Scanner: Completed scan of directory: " + directoryPath);
                logger.log(utils::LogLevel::INFO, "Scanner: Found " + std::to_string(indexedCode.fileCount) + " files with total size " + std::to_string(indexedCode.totalSize) + " bytes");

            }
            catch (const std::exception& e) {
                utils::Logger& logger = utils::Logger::getInstance();
                logger.log(utils::LogLevel::ERROR, "Scanner: Error scanning directory: " + std::string(e.what()));
            }
